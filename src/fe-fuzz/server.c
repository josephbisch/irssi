...
int LLVMFuzzerInitialize(int *argc, char ***argv) {
	...
	//test_server();
	signal_emit("server incoming", 2, server, "001 user\n");
	return 0;
}
...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
	...
	while (lines[i]) {
		gchar *prefixedLine;
		gboolean prefixedChoice = g_random_boolean();
		if (prefixedChoice) {
			GRand *rand = g_rand_new();
			gchar *nick = g_strdup("user");
			if (g_random_boolean()) {
				g_free(nick);
				nick = (char *)malloc(2);
				nick[0] = (char)g_rand_int_range(rand, 0x61, 0x7B);
				nick[1] = '\0';
			}
			prefixedLine = g_strdup_printf(":%s %s\n", nick, lines[i]);
			g_rand_free(rand);
			g_free(nick);
		} else {
